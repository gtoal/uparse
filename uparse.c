...
static wchar_t Matched_string[10240]; // Global because it is used in debug reports.
...
int parse(int *XTP, int P, int depth) { // depth is only used for indenting the debugging
  ...
  if (depth == 10000) {
    fprintf(stderr, "* ERROR: We appear to have runaway parse recursion - probably an undetected grammar loop from left-recursion.\n");
    // FORCE GDB CRASH
